...
def perturbation(data, labels, ord, ncyc):
    ...
    slt_features_vector = (slt_features_vector - np.mean(slt_features_vector)) / np.std(slt_features_vector)
    ...
    for num in range(RUNS):
        model, metrics = nn.apply_neural_network(slt_features_vector, labels)
    ...
    for id, correlated_set in enumerate(correlated_sets):
        ...
        for num in range(RUNS):
            ...
            model, metrics = nn.apply_neural_network(perturbed_features, labels)
